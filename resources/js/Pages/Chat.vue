    ...
    <app-layout>
        ...
        <div class="py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <p class="w-full py-4 text-lg text-gray-600 font-semibold">Usuário: {{$page.auth.user.name}}</p>
...
<!--                            @click="() => {loadMessages(user.id)}"-->
                            <li v-for="user in users" :key="user.id"
                                ...
                                class="p-6 text-lg text-gray-600 leading-7 font-semibold border-b hover:bg-gray-200 hover:bg-opacity-50 hover:cursor-pointer">
                                <p class="flex items-center">{{ user.name }}<span v-if="user.notification" class="ml-2 px-2 py-0 bg-green-500 text-xs text-white text-center rounded-full">{{user.notificationQtd}}</span></p>
                            </li>
                    ...
                    <div class="w-9/12 flex flex-col justify-between">
                        ...
                        <!-- message-->
                        <div class="w-full flex-1 p-6 flex flex-col overflow-y-auto">
                            ...
                            <div v-for="message in messages" :key="message.id"
                                ...
                                <p :class="(message.from == $page.auth.user.id ? 'messageFromMe' : 'messageToMe')"
                                    class="inline-block p-2 rounded-md" style="max-width: 75%;">{{message.content}}</p>
        ...
        methods: {
            ...
            loadMessages: async function(userId){
                ...
                if(user){
                    ...
                    //Vue.set(user[0], 'notificationQtd', 0)
                }
        ...
        mounted(){
            ...
            Echo.private(`user.${this.user.id}`).listen('.SendMessage', async (e) => {
                ...
                if(this.userActive && this.userActive.id === e.message.from){
                    ...
                    if(user){
                        // user.notification = true [Não é reativo]
                        Vue.set(user[0], 'notification', true)
                        if(user[0].notificationQtd){
                            user[0].notificationQtd++;
                        }else{
                            Vue.set(user[0], 'notificationQtd', 1)
                        }
                    }
                }
            })
